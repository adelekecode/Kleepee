...
export function createIceServerLoader(
  workerBase: string,
  sessionId: string,
  deviceId: string,
) {
  ...
  return (): Promise<RTCIceServer[]> => {
    if (cached && cached.expiresAt > Date.now() + 30_000)
      return Promise.resolve(cached.iceServers);
