...
import type { FileFrame, FileItem, FileStartFrame, FileTransport } from "../types/files";

const device = { deviceId: "device-a", deviceName: "Calm Otter" };
const managers: FileTransferManager[] = [];
function manager() { const value = new FileTransferManager(); managers.push(value); return value; }
...
function item(value: FileTransferManager, id: string) { return items(value).find((entry) => entry.id === id); }
function start(patch: Partial<FileStartFrame> = {}): FileStartFrame {
  return { type: "file.start", id: "incoming-1", senderId: "device-b", senderName: "Bright Fox", timestamp: 1,
    fileName: "file.bin", mimeType: "application/octet-stream", fileSize: 1026, chunkSize: 1024, totalChunks: 2, ...patch };
}
function recordingTransport(frames: FileFrame[] = []): FileTransport {
  return { chunkSize: CHUNK_SIZE, send: async (frame) => { frames.push(frame); return true; } };
}
function receiver() {
  const value = manager(); const frames: FileFrame[] = [];
...
function connect(a: FileTransferManager, b: FileTransferManager) {
  ...
  a.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame, signal) => {
    if (signal.aborted) return false;
    frames.push(frame); b.handleFrame(frame); return true;
  } });
  b.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame, signal) => {
    if (signal.aborted) return false;
    a.handleFrame(frame); return true;
  } });
  return frames;
}
...
describe("file transfer delivery", () => {
  it.each([0, CHUNK_SIZE + 7, MAX_FILE_BYTES])("delivers and acknowledges exactly %i bytes", async (size) => {
    const a = manager(); const b = manager(); const frames = connect(a, b);
    const bytes = Uint8Array.from({ length: size }, (_, index) => index % 251);
    expect(a.enqueue([new File([bytes], "archive.custom")], device).errors).toEqual([]);
    await vi.waitFor(() => expect(items(a)[0]?.status).toBe("complete"), { timeout: 5000 });
    ...
    expect(new Uint8Array(await crypto.subtle.digest("SHA-256", downloaded)))
      .toEqual(new Uint8Array(await crypto.subtle.digest("SHA-256", bytes)));
    expect(frames.filter((frame) => frame.type === "file.chunk")).toHaveLength(Math.ceil(size / CHUNK_SIZE));
  });
  ...
  it("waits for receiver acknowledgement and sends only one queued file at a time", async () => {
    ...
    expect(frames.filter((frame) => frame.type === "file.start")).toHaveLength(1);
    expect(item(value, firstId)?.status).toBe("sending");
    expect(items(value).filter((entry) => entry.status === "queued")).toHaveLength(1);
    ...
    expect(frames.filter((frame) => frame.type === "file.start")).toHaveLength(2);
  });
  ...
  it("keeps files queued until a connection is available", async () => {
    ...
    value.setTransport(recordingTransport(frames));
    await flush();
    expect(frames.map((frame) => frame.type)).toEqual(["file.start", "file.complete"]);
  });

  it("marks a failed completion send as failed with a retained retry", async () => {
    const value = manager();
    value.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => frame.type !== "file.complete" });
  ...
  it("returns rejected attachments without hiding accepted files", () => {
    ...
    const result = value.enqueue([empty, new File([new Uint8Array(MAX_FILE_BYTES + 1)], "too-big")], device);
...
describe("incoming validation and cancellation", () => {
  it.each([
    ...
    ["wrong byte count", [{ type: "file.chunk", id: "incoming-1", index: 0, data: new Uint8Array(1) }]],
    ["oversized", [{ type: "file.chunk", id: "incoming-1", index: 0, data: new Uint8Array(1025) }]],
    ["out of order", [{ type: "file.chunk", id: "incoming-1", index: 1, data: new Uint8Array(1024) }]],
    ["duplicate", [{ type: "file.chunk", id: "incoming-1", index: 0, data: new Uint8Array(1024) }, { type: "file.chunk", id: "incoming-1", index: 0, data: new Uint8Array(1024) }]],
    ["incorrect final size", [{ type: "file.chunk", id: "incoming-1", index: 0, data: new Uint8Array(1024) }, { type: "file.chunk", id: "incoming-1", index: 1, data: new Uint8Array(1) }, { type: "file.complete", id: "incoming-1" }]],
    ["non-binary", [{ type: "file.chunk", id: "incoming-1", index: 0, data: "AAAA" }]],
    ...
    expect(item(value, "incoming-1")).toMatchObject({ status: "failed", canRetry: false });
  ...
  it("rejects malicious metadata and duplicate starts without allocating a download", () => {
    ...
    value.handleFrame(start()); value.handleFrame(start());
    expect(item(value, "incoming-1")?.status).toBe("failed");
  });

  it.each(["sender", "receiver"])("propagates %s cancellation and ignores late chunks", async (side) => {
    const a = manager(); const b = manager();
    let release!: () => void;
    const blocked = new Promise<void>((resolve) => { release = resolve; });
    a.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => {
      if (frame.type === "file.start") { b.handleFrame(frame); await blocked; }
      else b.handleFrame(frame);
      return true;
    } });
    b.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => { a.handleFrame(frame); return true; } });
    ...
    b.handleFrame({ type: "file.chunk", id, index: 0, data: new TextEncoder().encode("hello") });
...
describe("connection lifecycle and cleanup", () => {
  it("fails interrupted transfers and retries under a fresh ID after reconnect", async () => {
    const a = manager(); const b = manager(); const frames: FileFrame[] = [];
    a.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => {
      frames.push(frame); b.handleFrame(frame); return true;
    } });
  ...
  it("releases an incomplete receive on connection loss", () => {
    ...
    value.handleFrame(start()); value.setTransport(null);
    expect(item(value, "incoming-1")).toMatchObject({ status: "failed", canRetry: false });
    expect(item(value, "incoming-1")?.objectUrl).toBeUndefined();
  ...
  it("does not recreate state when asynchronous sends finish after clear", async () => {
    const value = manager(); let release!: () => void;
    const pending = new Promise<void>((resolve) => { release = resolve; });
    const frames: FileFrame[] = [];
    value.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => { frames.push(frame); await pending; return true; } });
    value.enqueue([new File(["pending"], "late.txt")], device);
    const id = items(value)[0].id;
    value.clear(); release(); await flush();
