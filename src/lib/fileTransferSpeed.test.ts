...
import { FileTransferManager, PREPARE_WINDOW, PROGRESS_INTERVAL_MS } from "./fileTransferManager";
...
import type { FileFrame, FileStartFrame, PreparedFileFrame } from "../types/files";
...
function controlledSender() {
  const value = manager();
  const preparations = new Map<number, ReturnType<typeof deferred<PreparedFileFrame>>>();
  const sent: number[] = [];
  value.setTransport({
    chunkSize: 1024,
    send: async (frame) => {
      if (frame.type === "file.complete") value.handleFrame({ type: "file.ack", id: frame.id });
      return true;
    },
    prepare: async (frame) => {
  ...
  const release = (index: number) => preparations.get(index)!.resolve({
    send: async () => { sent.push(index); return true; },
  });
  return { value, preparations, sent, reads, release };
}
function incomingStart(): FileStartFrame {
  return { type: "file.start", id: "speed-incoming", senderId: "peer", senderName: "Bright Fox", timestamp: 1,
    fileName: "progress.bin", mimeType: "application/octet-stream", fileSize: 4 * 1024, chunkSize: 1024, totalChunks: 4 };
}
...
describe("bounded file preparation", () => {
  ...
  it.each(["cancel", "disconnect", "reset"])("does not send prepared chunks after %s, including late failures", async (operation) => {
    ...
    else expect(items(value)[0].status).toBe(operation === "cancel" ? "cancelled" : "failed");
  });

  it("delivers an exact 4 MiB download through prepared encryption and receiver acknowledgement", async () => {
    const sender = manager(); const receiver = manager();
    const key = await deriveKey(generateSessionSecret());
    const prepared = vi.fn(async (frame: FileFrame, signal: AbortSignal): Promise<PreparedFileFrame> => {
      const encrypted = await encryptBytes(key, encodeFileFrame(frame));
      return { send: async () => {
        if (signal.aborted) return false;
        receiver.handleFrame(decodeFileFrame(await decryptBytes(key, encrypted)));
        return true;
      } };
    });
    sender.setTransport({ chunkSize: CHUNK_SIZE, prepare: prepared, send: async (frame, signal) => {
      return (await prepared(frame, signal)).send();
    } });
    receiver.setTransport({ chunkSize: CHUNK_SIZE, send: async (frame) => { sender.handleFrame(frame); return true; } });
    const bytes = Uint8Array.from({ length: 4 * 1024 * 1024 }, (_, index) => index % 251);
    sender.enqueue([new File([bytes], "four-mib.bin")], device);
    await vi.waitFor(() => expect(items(sender)[0]?.status).toBe("complete"), { timeout: 5000 });
    ...
    expect(prepared.mock.calls.filter(([frame]) => frame.type === "file.chunk")).toHaveLength(64);
  });
});

describe("coalesced file progress", () => {
  it.each(["complete", "failed", "cancelled", "clear"])("publishes progress at 100 ms and %s immediately without leaking a timer", async (terminal) => {
    vi.useFakeTimers();
    const value = manager();
    value.setTransport({ chunkSize: 1024, send: async () => true });
    const listener = vi.fn(); value.subscribe(listener);
    const start = incomingStart();
    value.handleFrame(start);
    expect(listener).toHaveBeenCalledTimes(1);
    for (let index = 0; index < 2; index++) value.handleFrame({ type: "file.chunk", id: start.id, index, data: new Uint8Array(1024) });
    expect(listener).toHaveBeenCalledTimes(1);
    ...
    value.handleFrame({ type: "file.chunk", id: start.id, index: 2, data: new Uint8Array(1024) });
    if (terminal === "complete") {
      value.handleFrame({ type: "file.chunk", id: start.id, index: 3, data: new Uint8Array(1024) });
