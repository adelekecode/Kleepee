...
describe("file feed", () => {
  it("keeps live files in chronological order alongside text without duplicate cards", () => {
    ...
    act(() =>
      root.render(
        <TextFeed
          ...
          transfers={new Map([[base.id, live]])}
        />,
      ),
    );
    ...
    expect(
      host.querySelector('[role="region"]')?.getAttribute("tabindex"),
    ).toBe("0");
  });
  ...
  it("offers cancel for queued transfers and retry only for retryable failures", () => {
    ...
    act(() =>
      root.render(
        <FileCard
          item={{ ...base, status: "queued", progress: 0 }}
          onCancel={cancel}
        />,
      ),
    );
    ...
    act(() =>
      root.render(
        <FileCard
          item={{ ...base, status: "failed", canRetry: false }}
          onRetry={retry}
        />,
      ),
    );
  ...
  it("only exposes downloads for complete files and keeps long names readable", () => {
    ...
    act(() =>
      root.render(
        <FileCard item={{ ...base, fileName, objectUrl: "blob:complete" }} />,
      ),
    );
