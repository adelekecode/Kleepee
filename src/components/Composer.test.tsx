...
describe("Composer", () => {
  it("removes only accepted attachments and shows rejection guidance", async () => {
    ...
    const onSubmit = vi.fn(async () => ({
      textSent: false,
      accepted: [first],
      errors: ["Queue is full. Try again."],
    }));
  ...
  it("preserves new draft edits while an earlier message is pending", async () => {
    ...
    const onSubmit = vi.fn(
      () =>
        new Promise<ComposerResult>((done) => {
          resolve = done;
        }),
    );
    act(() => root.render(<Composer onSubmit={onSubmit} />));
    ...
    type("Next draft");
    await act(async () =>
      resolve({ textSent: true, accepted: [], errors: [] }),
    );
  ...
  it("blocks blank submissions and UTF-8 text beyond the byte limit", async () => {
    ...
    expect(host.querySelector("textarea")?.getAttribute("aria-invalid")).toBe(
      "true",
    );
    expect(host.querySelector('[role="alert"]')?.textContent).toContain(
      "65,536",
